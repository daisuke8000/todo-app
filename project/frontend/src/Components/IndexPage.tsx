...
export const IndexPage: VFC = memo(() => {
   ...
   const getTodos = useCallback(async() => {
         const response: Response = await fetch(`http://${process.env.REACT_APP_API_SERVICE_URL}/todos`)
         await response.json()
             .then((r) => {
                setTodos(r)
             })
   ...
   const onClickAddTodo = () => {
      ...
      fetch(`http://${process.env.REACT_APP_API_SERVICE_URL}/todos`, {
         method: "POST",
...
const UpdateTodo: VFC<UpdateProps> = (props) => {
   ...
   const onClickUpdateTodo  = async(id: number, item: string) => {
      await fetch(`http://${process.env.REACT_APP_API_SERVICE_URL}/todos/${id}`, {
         method: "PUT",
         headers: { "Content-Type": "application/json" },
